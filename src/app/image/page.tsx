...
export default function ImageStudioChat() {
  ...
  return (
    <div className="flex h-[calc(100vh-72px)] transition-colors duration-300 relative" style={{ background: bg, color: fg }}>
      
      {/* Left Sidebar - Desktop */}
      <aside className="w-[300px] shrink-0 flex flex-col hidden md:flex border-r relative z-10" style={{ background: surface, borderColor: border }}>
        {renderSidebarContent()}
      </aside>

      {/* Left Sidebar - Mobile Overlay */}
      {mobileHistoryOpen && (
        <div className="md:hidden fixed inset-0 z-50 bg-black/40 backdrop-blur-sm" onClick={() => setMobileHistoryOpen(false)}>
          <div 
            className="absolute top-0 bottom-0 left-0 w-[280px] shadow-2xl flex flex-col border-r animate-in slide-in-from-left-full"
      ...
      <main className="flex-1 flex flex-col relative min-w-0 bg-transparent">
        ...
        {/* Mobile Header */}
        <div className="md:hidden flex items-center p-3 border-b shrink-0 bg-transparent backdrop-blur-md sticky top-0 z-10" style={{ borderColor: border }}>
          <button className="p-2 -ml-1 rounded-lg hover:bg-black/5 dark:hover:bg-white/5" onClick={() => setMobileHistoryOpen(true)}>
          ...
          <span className="ml-2 font-bold text-sm">Image Studio</span>
        </div>
